      ...
      case request @ GET -> Root / "api" / "problems" / rawProblemSlug / "blogs" =>
        ProblemSlug.parse(rawProblemSlug) match
          ...
          case Right(problemSlug) =>
            handlers.execute(request, BlogHttpPlans.ProblemBlogsInput(problemSlug, PageRequestQuerySupport.parsePageRequest(request.uri.query.params)), plans.listProblemBlogs)
    }
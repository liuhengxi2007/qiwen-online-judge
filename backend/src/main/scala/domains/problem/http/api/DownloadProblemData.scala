  ...
  def routes(context: ProblemHttpRouteContext)(using Http4sDsl[IO]): HttpRoutes[IO] =
    HttpRoutes.of[IO] {
      case request @ GET -> Root / "api" / "problems" / problemSlug / "data" / filename =>
        ProblemHttpRequestMappers.problemSlugAndFilename(problemSlug, filename) match
          case Left(message) =>
            ProblemHttpResponseMappers.validationErrorResponse(message)
          case Right(input) =>
            context.handlers.execute(request, input, context.plans.downloadProblemData)
    }

...
object ProblemDataApiSupport:
  ...
  def withManageableProblemForUpdate[A](
    ...
    ProblemQueryTable.findBySlugForUpdate(connection, problemSlug).flatMap {
      case None =>
        shared.http.HttpApiError.raise(shared.http.HttpApiError.notFound(shared.http.ApiMessages.problemNotFound))
      case Some(problem) =>
        ProblemAccessRules.canManageProblem(connection, actor, problem).flatMap { canManage =>
          shared.http.HttpApiError.ensure(
            canManage,
            shared.http.HttpApiError.notFound(shared.http.ApiMessages.problemNotFound)
          ) *> useProblem(problem)
        }
    }
  ...
  def refreshedManagedProblem(
    ...
    ProblemQueryTable
      ...
      .flatMap {
        ...
        case None => shared.http.HttpApiError.raise(shared.http.HttpApiError.internal(missingMessage))
      }

...
object MessageMutationCommands:
  ...
  def createConversation(
    ...
    if actor.username == request.targetUsername then IO.pure(CreateConversationResult.CannotMessageSelf)
      ...
      MessageTable.userExists(connection, request.targetUsername).flatMap {
  ...
  def sendMessage(
    ...
    MessageTable.findOtherParticipant(connection, actor.username, conversationId).flatMap {
      case None =>
        IO.pure(SendMessageResult.ConversationNotFound)
      case Some(recipientUsername) =>
        MessageTable.isBlocked(connection, recipientUsername, actor.username).flatMap {
  ...
  def markConversationRead(
    ...
    val _ = request
    MessageTable.findConversationSummaryForUser(connection, actor.username, conversationId).flatMap {
      ...
      case Some(summary) =>
        MessageTable.findOtherParticipant(connection, actor.username, conversationId).flatMap {
          ...
          case Some(otherParticipant) =>
            MessageTable.markConversationRead(connection, conversationId, actor.username).map { readUpToMessageId =>
  ...
  def addBlock(
    ...
    if actor.username == targetUsername then IO.pure(AddBlockResult.CannotBlockSelf)
      ...
      MessageTable.userExists(connection, targetUsername).flatMap {
        case false => IO.pure(AddBlockResult.TargetUserNotFound)
        case true => MessageTable.upsertBlock(connection, actor.username, targetUsername).map(AddBlockResult.Added(_))
      }
  ...
  def removeBlock(
    ...
    MessageTable.removeBlock(connection, actor.username, targetUsername).as(RemoveBlockResult.Removed)

...
import domains.auth.http.{AuthApiSupport, AuthenticatedResponseApi}
...
final case class UpdateAccount(sessionStore: SessionStore) extends AuthenticatedResponseApi[(Username, Json)]:
  ...
  private def updateAccountRecord(
    ...
    yield
      val response = Response[IO](status = Status.Ok).withEntity(AuthApiSupport.toSessionResponse(user).asJson)
      if passwordChanged && clearSessionOnPasswordChange then response.addCookie(AuthApiSupport.clearedSessionCookie)
      else response

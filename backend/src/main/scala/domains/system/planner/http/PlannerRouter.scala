...
object PlannerRouter:
  ...
  def routes(databaseSession: DatabaseSession): HttpRoutes[IO] =
    def decodeInput[Input](plannerName: String, payload: Json)(using decoder: io.circe.Decoder[Input]): IO[Input] =
      IO.fromEither(
    ...
    def executePlanner(plannerName: String, payload: Json): IO[Json] =

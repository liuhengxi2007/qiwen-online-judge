...
class JudgeApiSupportSuite extends CatsEffectSuite:
  ...
  test("ensureJudgeToken authorizes requests with the configured token") {
    ...
    JudgeApiSupport.ensureJudgeToken(request, config).map(result => assertEquals(result, ()))
  }

  test("ensureJudgeToken rejects requests with a missing token") {
    val request = Request[IO]()

    JudgeApiSupport.ensureJudgeToken(request, config).attempt.map {

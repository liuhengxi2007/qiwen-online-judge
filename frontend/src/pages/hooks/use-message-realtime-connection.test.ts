...
async function loadHarness() {
  ...
  vi.doMock('@/pages/hooks/use-message-inbox-refresh', () => ({
    useMessageInboxRefresh: () => refreshInbox,
  }))

  const authStoreModule = await import('@/pages/stores/auth/use-auth-store')
  const messageStoreModule = await import('@/pages/stores/message/use-message-store')
  const hookModule = await import('@/pages/hooks/use-message-realtime-connection')

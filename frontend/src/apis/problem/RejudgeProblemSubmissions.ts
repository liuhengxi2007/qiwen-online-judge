...
/** 手动整题重判；输入题目 slug 和可选比赛上下文，输出通用成功响应。 */
export class RejudgeProblemSubmissions implements APIWithSessionMessage<SuccessResponse> {
  ...
  constructor(problemSlug: ProblemSlug, contestSlug?: ContestSlug) {
    ...
    this.apiPath = `problems/${problemSlugValue(problemSlug)}/submissions/rejudge${params.size > 0 ? `?${params.toString()}` : ''}`
  }

...
wsl.exe bash -lc "if [ -n '%BACKEND_BASE_URL%' ]; then export BACKEND_BASE_URL='%BACKEND_BASE_URL%'; fi && cd \"$(wslpath '%JUDGER_WIN_PATH%')\" && chmod +x ./run-wsl.sh && ./run-wsl.sh; exec bash"
